...
const SecuritySettingsTab: React.FC<Props> = ({ control, watch }) => {
  ...
  const sessionManagementEnabled = watch(
    "security.sessionManagement.isEnabled",
  );
  const sessionFingerprintValidationEnabled = watch(
    "security.sessionManagement.isSessionFingerprintValidationEnabled",
  );
  ...
  return (
    <Form layout="vertical">
        ...
        <Controller
          ...
          render={({ field }) => (
            <div className="form-check form-check-custom form-check-solid">
              ...
              >
                {L("EnableCheckingLastXPasswordWhenPasswordChangeSettingLabel")}
              </label>
            </div>
          )}
        />
      ...
      {enableCheckingLastXPassword && (
        <div className="mb-5">
          <label className="form-label" htmlFor="CheckingLastXPasswordCount">
      ...
      {enablePasswordExpiration && (
        <>
          <div className="mb-5">
            <label className="form-label" htmlFor="PasswordExpirationDayCount">
      ...
      <h5 className="mt-5 mb-3">{L("SessionManagement")}</h5>
      <Form.Item>
        <Controller
          name="security.sessionManagement.isEnabled"
          control={control}
          render={({ field }) => (
            <div className="form-check form-check-custom form-check-solid">
              <input
                ...
                checked={field.value}
                onChange={field.onChange}
                id="sessionManagementIsEnabled"
              />
              <label
                className="form-check-label"
                htmlFor="sessionManagementIsEnabled"
              >
                {L("SessionManagement_IsEnabled")}
              </label>
            </div>
          )}
        />
      ...
      {sessionManagementEnabled && (
        <>
          <Form.Item>
            <Controller
              name="security.sessionManagement.isSessionFingerprintValidationEnabled"
              control={control}
              render={({ field }) => (
                <div className="form-check form-check-custom form-check-solid">
                  <input
                    ...
                    checked={field.value}
                    onChange={field.onChange}
                    id="sessionFingerprintValidationEnabled"
                  />
                  <label
                    className="form-check-label"
                    htmlFor="sessionFingerprintValidationEnabled"
                  >
                    {L("SessionFingerprintValidation")}
                  </label>
                </div>
              )}
            />
          </Form.Item>
          {sessionFingerprintValidationEnabled && (
            <div className="mb-5">
              <label
                className="form-label"
                htmlFor="sessionFingerprintValidationPolicy"
              >
                {L("SessionFingerprintValidationPolicy")}
              </label>
              <Controller
                name="security.sessionManagement.sessionFingerprintValidationPolicy"
                control={control}
                render={({ field }) => (
                  <select
                    id="sessionFingerprintValidationPolicy"
                    className="form-select"
                    value={field.value ?? "IpAndUserAgent"}
                    onChange={field.onChange}
                  >
                    <option value="IpAndUserAgent">
                      {L("SessionFingerprintPolicy_IpAndUserAgent")}
                    </option>
                    <option value="UserAgentOnly">
                      {L("SessionFingerprintPolicy_UserAgentOnly")}
                    </option>
                    <option value="IpOnly">
                      {L("SessionFingerprintPolicy_IpOnly")}
                    </option>
                  </select>
                )}
              />
          ...
          <div className="mb-5">
            <label
              className="form-label"
              htmlFor="sessionAbsoluteTimeoutMinutes"
            >
              {L("SessionAbsoluteTimeoutMinutes")}
            </label>
            <Controller
              name="security.sessionManagement.sessionAbsoluteTimeoutMinutes"
              control={control}
              render={({ field }) => (
                <input
                  id="sessionAbsoluteTimeoutMinutes"
                  type="number"
                  min={10}
                  max={43200}
                  {...field}
          ...
          <Form.Item>
            <Controller
              name="security.sessionManagement.isSessionRevocationEnabled"
              control={control}
              render={({ field }) => (
                <div className="form-check form-check-custom form-check-solid">
                  <input
                    ...
                    checked={field.value}
                    onChange={field.onChange}
                    id="sessionRevocationEnabled"
                  />
                  <label
                    className="form-check-label"
                    htmlFor="sessionRevocationEnabled"
                  >
                    {L("SessionRevocation")}
                  </label>
                </div>
