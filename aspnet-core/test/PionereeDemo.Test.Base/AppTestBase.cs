...
using PionereeDemo.Test.Base.TestData;
...
/// Provides methods to easily work with <see cref="PionereeDemoDbContext"/>.
/// </summary>
public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
{
    ...
    {
        void NormalizeDbContext(PionereeDemoDbContext context)
        {
    ...
    protected void UsingDbContext(Action<PionereeDemoDbContext> action)
    {
        UsingDbContext(AbpSession.TenantId, action);
    }

    protected Task UsingDbContextAsync(Func<PionereeDemoDbContext, Task> action)
    {
        return UsingDbContextAsync(AbpSession.TenantId, action);
    }

    protected TResult UsingDbContext<TResult>(Func<PionereeDemoDbContext, TResult> func)
    {
        return UsingDbContext(AbpSession.TenantId, func);
    }

    protected Task<TResult> UsingDbContextAsync<TResult>(Func<PionereeDemoDbContext, Task<TResult>> func)
    {
        return UsingDbContextAsync(AbpSession.TenantId, func);
    }

    protected void UsingDbContext(int? tenantId, Action<PionereeDemoDbContext> action)
    {
        using (UsingTenantId(tenantId))
        {
            using (var context = LocalIocManager.Resolve<PionereeDemoDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
    ...
    protected async Task UsingDbContextAsync(int? tenantId, Func<PionereeDemoDbContext, Task> action)
    {
        using (UsingTenantId(tenantId))
        {
            using (var context = LocalIocManager.Resolve<PionereeDemoDbContext>())
    ...
    protected TResult UsingDbContext<TResult>(int? tenantId, Func<PionereeDemoDbContext, TResult> func)
    {
        TResult result;
    ...
    protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<PionereeDemoDbContext, Task<TResult>> func)
    {
        TResult result;
