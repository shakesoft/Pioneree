...
namespace PionereeDemo.Authentication.TwoFactor.Google;

public class GoogleAuthenticatorProvider : PionereeDemoServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
{
    ...
    public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
    {
        ...
        var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("PionereeDemo", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
