...
using PionereeDemo.Authentication.PasswordlessLogin;
...
namespace PionereeDemo.Authorization.Accounts;

public class AccountAppService : PionereeDemoAppServiceBase, IAccountAppService
{
    ...
    public async Task SendEmailActivationLink(SendEmailActivationLinkInput input)
    {
        ...
        if (user == null)
        {
            throw new AbpException(L("UserNotFoundForEmail"));
        }
    ...
    private async Task SendSmsPasswordlessCode(string phoneNumber)
    {
        ...
        var message = string.Format(L("PasswordlessLogin_SmsMessage", PionereeDemoConsts.ProductName, code));
