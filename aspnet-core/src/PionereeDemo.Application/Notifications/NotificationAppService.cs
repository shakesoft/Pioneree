...
using PionereeDemo.Organizations;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace PionereeDemo.Notifications;

[AbpAuthorize]
public class NotificationAppService : PionereeDemoAppServiceBase, INotificationAppService
{
    ...
    public async Task<NotificationDetailDto> GetNotificationDetail(EntityDto<Guid> input)
    {
        ...
        return new NotificationDetailDto
        {
            ...
            Message = ResolveMessage(notification.Data),
        };
    }


    private string ResolveMessage(NotificationData data)
    {
        if (data == null)
            return string.Empty;

        if (data is LocalizableMessageNotificationData localizable)
        {
            var text = _localizationManager.GetString(
                localizable.Message.SourceName,
                localizable.Message.Name
            );

            if (localizable.Properties == null || localizable.Properties.Count == 0)
                return text;

            foreach (var (key, value) in localizable.Properties)
            {
        ...
        if (data is MessageNotificationData messageData)
            return messageData.Message;

        var messageProp = data.GetType().GetProperty("Message");
        return messageProp?.GetValue(data)?.ToString() ?? data.ToString();
    }
}
